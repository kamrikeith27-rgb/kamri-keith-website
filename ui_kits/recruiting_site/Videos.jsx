...
function Videos() {
  ...
  const [selectedClip, setSelectedClip] = React.useState(CURATED_CLIPS[0]);
      ...
      <div style={{ maxWidth: KK.wrap, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, justifyContent: 'center', marginBottom: 32, marginTop: selectedClip ? 0 : 32 }}>
          <button 
            ...
            style={{
              fontFamily: KK.head, fontWeight: 600, fontSize: 13, textTransform: 'uppercase', letterSpacing: '.05em',
              padding: '8px 16px', border: 'none', borderRadius: 4,
              background: activeTab === 'all' ? KK.red : '#22262d', color: activeTab === 'all' ? '#fff' : '#aaa',
          ...
          <button 
            ...
            style={{
              fontFamily: KK.head, fontWeight: 600, fontSize: 13, textTransform: 'uppercase', letterSpacing: '.05em',
              padding: '8px 16px', border: 'none', borderRadius: 4,
              background: activeTab === 'hitting' ? KK.red : '#22262d', color: activeTab === 'hitting' ? '#fff' : '#aaa',
              cursor: 'pointer', transition: 'all .2s ease'
            }}
          >Hitting</button>
          <button 
            ...
            style={{
              fontFamily: KK.head, fontWeight: 600, fontSize: 13, textTransform: 'uppercase', letterSpacing: '.05em',
              padding: '8px 16px', border: 'none', borderRadius: 4,
              background: activeTab === 'fielding' ? KK.red : '#22262d', color: activeTab === 'fielding' ? '#fff' : '#aaa',
              cursor: 'pointer', transition: 'all .2s ease'
            }}
          >Fielding</button>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 280px), 1fr))', gap: 16, maxWidth: 900, marginInline: 'auto' }}>
          {filteredClips.map((clip, i) => (
            <button
              ...
              style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', textDecoration: 'none', textAlign: 'left' }}
            >
              <div style={{ position: 'relative', aspectRatio: '16 / 10', overflow: 'hidden', borderRadius: 8, border: selectedClip.id === clip.id ? '3px solid ' + KK.red : '1px solid #22262d' }}>
                <img src={ytThumbnail(clip.id)} alt={clip.label} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }} />
